    ...
    <div style={{
      ...
      {payload.map((p) => (
        <div key={p.dataKey} style={{ color: p.color, marginBottom: '4px', display: 'flex', justifyContent: 'space-between', gap: '16px' }}>
          <span>{p.name}:</span>
          <span style={{ fontWeight: '600' }}>{Number(p.value || 0).toLocaleString()}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={safeData} margin={{ top: 15, right: 10, left: 0, bottom: 5 }}>
        ...
        <Bar dataKey="order_count"    name="Orders" fill="url(#colorOrders)" radius={[6, 6, 0, 0]} maxBarSize={50} />
        <Bar dataKey="total_quantity" name="Qty"    fill="url(#colorQty)" radius={[6, 6, 0, 0]} maxBarSize={50} />
      </BarChart>
    </ResponsiveContainer>
  )
}
